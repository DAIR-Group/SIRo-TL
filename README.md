...
SIRo-TL is a Python package that implements a framework for conducting valid statistical inference for Outlier Detection in High-Dimensional Robust Regression under Transfer Learning (TL) scenarios.
The main idea is to synergize the inherent robustness of Huber regression with the rigorous guarantees of the Selective Inference framework. Unlike standard methods that merely flag observations with large residuals, SIRo-TL computes valid $p$-values to formally test whether a detected anomaly is a true outlier or merely random noise arising from heavy-tailed distributions. By leveraging auxiliary source domain information to stabilize estimation, our method ensures that the False Positive Rate (FPR) of outlier detection is strictly controlled while maximizing the True Positive Rate (TPR), offering a resilient solution for data cleaning in complex, high-stakes environments.
