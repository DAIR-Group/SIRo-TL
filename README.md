...
This repository contains the official Python implementation of SIRo-TL, a framework for conducting valid statistical inference for Outlier Detection in High-Dimensional Robust Regression under Transfer Learning (TL) scenarios. The core philosophy is to synergize the inherent robustness of Huber regression with the rigorous guarantees of the Selective Inference framework. Unlike standard methods that merely flag observations based on large residuals, SIRo-TL computes valid p-values to formally test whether a detected anomaly is a true outlier or merely random noise arising from heavy-tailed distributions. By leveraging auxiliary source domain information to stabilize estimation, our method ensures that the False Positive Rate (FPR) is strictly controlled while maximizing detection power (TPR), offering a resilient solution for data cleaning in complex, high-stakes environments.
