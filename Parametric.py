...
def triple_parametric(Z, Z0, Y0_mask, lambda_w, lambda_d, alpha, gamma, a_y, b_y, xi_threshold, zk_threshold):
	N, p = Z.shape
	nT = Z0.shape[0]
	zk = -zk_threshold
	...
	while zk < zk_threshold:
		...
		Yz = Yz.ravel()
		Y0z = Y0_mask @ Yz
		Pw_z, Kw_z, q0w_z, q1w_z, Sw_z, hw_z, u0w_z, u1w_z = Gen_data.Construct_matrix(Z, Yz, gamma)
		...
		info_w = TL_Training.training_qp_w(Pw_z, Kw_z, q0w_z, q1w_z, Sw_z, hw_z, Z, lambda_w, alpha)
		if info_w == None:
		...
		Pd_z, Kd_z, q0d_z, q1d_z, Sd_z, hd_z, u0d_z, u1d_z = Gen_data.Construct_matrix(Z0, Y0z, gamma)

		hd_z[: nT] = Y0z - Z0 @ w_z
		hd_z[nT : 2 * nT] = - (Y0z - Z0 @ w_z)
		hd_z[3 * nT : 4 * nT] = gamma * np.ones(nT)

		info_d = TL_Training.training_qp_delta(Pd_z, Kd_z, q0d_z, q1d_z, Sd_z, hd_z, Z0, lambda_d, alpha)
		if info_d == None:
			return None, None, None
		...
		tr_w, A_w, b_w = Compute_step_size.compute_interval(Q_wz, f_wz, S_Az, S_Azc, hw_z, u0w_z, u1w_z, Z, t_wz, vw_Az, Az, Azc)
		...
		tr_d, A_d, b_d = Compute_step_size.compute_interval(Q_dz, f_dz, S_Oz, S_Ozc, hd_z, u0d_z, u1d_z, Z0, t_dz, vd_Oz, Oz, Ozc)
	...
	return list_zk, list_g_beta, list_l_beta